    ...
    def encode(v: Any) -> str:
        ...
        if isinstance(v, dict):
            ...
            for k in sorted(v.keys()):
                val = v[k]
                if val is None:
                    out.append(f"{json.dumps(str(k), ensure_ascii=False)}:null")
                elif isinstance(val, type(...)):
                    raise ValueError(f'canonicalize: unsupported value for key "{k}"')
                else:
    ...
    try:
        ...
        if public_key:
            ...
            res = resolve_ens_ed25519_pubkey(
                name=ens["name"],
                rpc_url=ens["rpcUrl"],
                pubkey_text_key=ens.get("pubkeyTextKey", "cl.pubkey"),
            )
        ...
        if not alg_matches:
            ...
            signature_error = ens_error or "no public key available (provide public_key or ens)"
        else:
            try:
