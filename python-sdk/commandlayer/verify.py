...
def extract_receipt(subject: CanonicalReceipt | CommandResponse) -> CanonicalReceipt:
    if isinstance(subject, dict) and isinstance(subject.get("receipt"), dict):
        subject_obj = cast(dict[str, Any], subject)
        return cast(CanonicalReceipt, dict(subject_obj["receipt"]))
...
def extract_receipt_verb(subject: CanonicalReceipt | CommandResponse) -> str | None:
    ...
    if isinstance(receipt_obj.get("verb"), str):
        return str(receipt_obj["verb"])
    x402 = receipt_obj.get("x402")
    if isinstance(x402, dict) and isinstance(x402.get("verb"), str):
        return str(x402["verb"])
    result = receipt_obj.get("result")
    if isinstance(result, dict):
        if isinstance(result.get("summary"), str):
            return "summarize"
        if isinstance(result.get("classification"), (str, dict, list)):
            return "classify"
        if isinstance(result.get("analysis"), (str, dict, list)):
            return "analyze"
        if isinstance(result.get("parsed"), (dict, list, str)):
            return "parse"
        if isinstance(result.get("cleaned"), str):
            return "clean"
        if isinstance(result.get("converted"), (str, dict)):
            return "convert"
        if isinstance(result.get("description"), str):
            return "describe"
        if isinstance(result.get("explanation"), str):
            return "explain"
        if isinstance(result.get("formatted"), str):
            return "format"
        if isinstance(result.get("fetched"), (str, dict, list)):
            return "fetch"
    return None
...
def verify_receipt(
    ...
    try:
        ...
        receipt_id_matches = not receipt_id or isinstance(receipt_id, str)
